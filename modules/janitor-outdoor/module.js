 ...
 * Dream OS v2.0 — Modul Janitor Outdoor (REVISED)
 ...
 * ✅ Assignment: 2 orang per blok area (Depan/Parkir, Belakang/Lapangan)
...
function injectCSS() {
    ...
    s.textContent = `
        ...
        .jo-tabs { display: flex; gap: 0.5rem; border-bottom: 2px solid var(--jo-primary-border); margin-bottom: 1.5rem; overflow-x: auto; scrollbar-width: none; }
        ...
        .jo-tab.active { background: rgba(6,182,212,0.18); border-color: var(--jo-primary); color: var(--jo-primary); }        .jo-form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem; }
        ...
        .jo-loader { display: flex; flex-direction: column; align-items: center; padding: 3rem; }
        ...
        @media print { #janitor-out-root { background: white; color: #1e293b; padding: 0.5in; } .jo-panel { background: white; backdrop-filter: none; border: 1px solid #ccc; box-shadow: none; } .jo-tabs, .jo-btn, .jo-header::before { display: none; } }
        .tier-low .jo-panel { backdrop-filter: none; background: rgba(15,23,42,0.95); }
        .tier-low .jo-spinner { animation: none; }
    `;
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    function renderRoot(container) {
        ...
        <div id="janitor-out-root">
            ...
            <div class="jo-panel jo-header" style="margin-bottom:1.5rem">
                <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">                    <div style="font-size:3rem;">🌿</div>
                    <div>
                        <div class="jo-title">JANITOR OUTDOOR</div>
                        <div style="font-size:0.75rem;color:var(--jo-text-muted);">Ceklis Harian · 2 Petugas · Blok Area</div>
                    </div>
            ...
            <!-- INFO BOX -->
            <div class="jo-panel" style="background:rgba(6,182,212,0.08);border-color:var(--jo-primary);margin-bottom:1.5rem;">
                <div style="font-size:0.85rem;color:var(--jo-text);">
                ...
                <div class="jo-panel">
                    ...
                    <form id="joForm">
                        <div class="jo-form-grid">
                            ...
                            <div>
                                <label class="jo-label">Ceklis Laporan *</label>
                            ...
                            <div>
                                ...
                                <input type="text" id="jo-petugas" class="jo-input" placeholder="Nama petugas" value="${esc(_user?.name||'')}" required>                            </div>
                            <div>
                                <label class="jo-label">Area / Blok *</label>
                                <input type="text" id="jo-area" class="jo-input" placeholder="Contoh: Area Depan & Parkir" required>
                                <span style="font-size:0.65rem;color:var(--jo-text-muted);">Wajib diisi (sesuai penugasan)</span>
                            </div>
                            <div>
                                <label class="jo-label">Gedung</label>
                                <input type="text" id="jo-gedung" class="jo-input" placeholder="Outdoor" value="Outdoor" readonly>
                            </div>
                            <div>
                                <label class="jo-label">Lantai / Zona</label>
                                <input type="text" id="jo-lantai" class="jo-input" placeholder="-" value="-" readonly>
                            </div>
                        </div>
            ...
            <div id="jo-history-tab" class="tab-content" style="display:none;">
                <div class="jo-panel">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
                        ...
                        <button id="jo-refresh-history" class="jo-btn jo-btn-sm"><i class="fas fa-sync-alt"></i> Refresh</button>                    </div>
            ...
            <div id="jo-schedule-tab" class="tab-content" style="display:none;">
                <div class="jo-panel">
                    <h3 style="font-size:1.2rem;font-weight:700;margin-bottom:1.5rem;color:var(--jo-primary);">📅 Penugasan Petugas Outdoor</h3>
    ...
    async function loadHistory() {
        ...
        if (!tbody || !_sb) return;
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;"><div class="jo-spinner" style="margin:0 auto;"></div><p style="margin-top:1rem;">Memuat...</p></td></tr>';        try {
    ...
    function loadSchedule() { /* Static, no dynamic load needed */ }
    ...
    async function handleSubmit(e) {
        ...
        const area = document.getElementById('jo-area')?.value.trim(); // REQUIRED!
    ...
    function switchTab(tab) {
        ...
        const activeTab = document.querySelector(`.jo-tab[data-tab="${tab}"]`);        if (activeTab) activeTab.classList.add('active');
