 ...
 * Dream OS v2.0 — Modul Janitor Indoor (REVISED)
 * ✅ Signature: (config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang)
 * ✅ Fitur: Form ceklis kebersihan indoor, riwayat, jadwal
 * ✅ Workflow: Semua kerja 06:30-17:00, ceklis laporan 3x: pagi/siang/sore
 * ✅ Assignment: 11 orang (SD:3, SMP:4, SMA:3, Serbaguna:1) per gedung & lantai
 * ✅ Database: area = REQUIRED (NOT NULL)
 */
...
function injectCSS() {
    ...
    s.textContent = `
        ...
        .ji-btn-primary {
            background: linear-gradient(135deg, var(--ji-primary), #0d9488);            color: #020617;
            width: 100%;
        }
        ...
        @media print {
            #janitor-in-root { background: white; color: #1e293b; padding: 0.5in; }
            .ji-panel { background: white; backdrop-filter: none; border: 1px solid #ccc; box-shadow: none; }
            .ji-tabs, .ji-btn, .ji-header::before { display: none; }
        }
        .tier-low .ji-panel { backdrop-filter: none; background: rgba(15,23,42,0.95); }
        .tier-low .ji-spinner { animation: none; }
    `;
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    const toast = showToast || function(msg, type) {
        ...
        el.innerHTML = `<span>${icons[type]||'ℹ️'}</span><span>${msg}</span>`;        const tc = document.getElementById('toast-container');
        if (tc) {
            tc.appendChild(el);
    ...
    function renderRoot(container) {
        ...
        <div id="janitor-in-root">
            ...
            <div class="ji-panel ji-header" style="margin-bottom:1.5rem">
                <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.75rem;color:var(--ji-text-muted);">Ceklis Harian · 11 Petugas · Gedung & Lantai</div>
                    </div>
            ...
            <!-- INFO BOX -->
            <div class="ji-panel" style="background:rgba(20,184,166,0.08);border-color:var(--ji-primary);margin-bottom:1.5rem;">
                <div style="font-size:0.85rem;color:var(--ji-text);">
                    ...
                    <form id="jiForm">
                        <div class="ji-form-grid">
                            <div>
                                ...
                                <label class="ji-label">Ceklis Laporan *</label>
                            ...
                            <div>
                                <label class="ji-label">Area / Lokasi *</label>
                                <input type="text" id="ji-area" class="ji-input" placeholder="Contoh: SD - Lantai 1" required>
                                <span style="font-size:0.65rem;color:var(--ji-text-muted);">Wajib diisi (sesuai penugasan)</span>
                            </div>
                            <div>
                                <label class="ji-label">Gedung</label>
                                <input type="text" id="ji-gedung" class="ji-input" placeholder="SD / SMP / SMA / Serbaguna">
                            </div>
                            <div>
                                <label class="ji-label">Lantai</label>
                                <input type="text" id="ji-lantai" class="ji-input" placeholder="1 / 2 / 3 / 3-4">
                            </div>
                        </div>
                        ...
                        <div style="margin-top:1.5rem;">
                            ...
                            <div class="ji-checkbox-grid" id="ruang-grid">
                                <label><input type="checkbox" id="ruang_loby_utama"> Loby Utama</label>                                <label><input type="checkbox" id="ruang_teras"> Teras</label>
            ...
            <div id="ji-schedule-tab" class="tab-content" style="display:none;">
                <div class="ji-panel">
                    <h3 style="font-size:1.2rem;font-weight:700;margin-bottom:1.5rem;color:var(--ji-primary);">📅 Penugasan Petugas Indoor</h3>
                    <div class="ji-table-wrap">
                        <table class="ji-table">
                            <thead>
                                <tr>                                    <th>Gedung</th>
                            ...
                            <tbody>
                                ...
                                <tr><td>SD</td><td>2</td><td>Siti</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SD</td><td>3</td><td>Budi</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMP</td><td>1</td><td>Dewi</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMP</td><td>2</td><td>Eko</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMP</td><td>3</td><td>Fitri</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMP Baru</td><td>1</td><td>Gunawan</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMA</td><td>1</td><td>Hadi</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMA</td><td>2</td><td>Indah</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>SMA</td><td>3-4</td><td>Joko</td><td>3x (Pagi, Siang, Sore)</td></tr>
                                <tr><td>Serbaguna</td><td>1</td><td>Karni</td><td>3x (Pagi, Siang, Sore)</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    /* ============================================================
       LOAD HISTORY — UPDATED TO SHOW AREA
    ============================================================ */
    ...
    /* ============================================================
       LOAD SCHEDULE — STATIC ASSIGNMENT
    ============================================================ */
    function loadSchedule() {
        // Already rendered in HTML, no dynamic load needed
    }

    /* ============================================================
       SUBMIT FORM — UPDATED WITH AREA REQUIRED
    ============================================================ */
    async function handleSubmit(e) {
        e.preventDefault();

        const tanggal = document.getElementById('ji-tanggal')?.value;
        const shift = document.getElementById('ji-shift')?.value; // pagi/siang/sore
        const petugas = document.getElementById('ji-petugas')?.value.trim();
        const area = document.getElementById('ji-area')?.value.trim(); // REQUIRED!
        ...
        try {
            ...
            // Catat ke audit_logs
            await writeAuditLog('Janitor Indoor', `Ceklis ${area} - ${shift} oleh ${petugas}`);

            toast('Ceklis indoor berhasil disimpan!', 'success');            document.getElementById('jiForm').reset();
