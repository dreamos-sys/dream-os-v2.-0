 ...
 * Dream OS v2.0 - Booking Module (REVISED)
 * ✅ Added: Sound Portable & Sound Display Karaoke
 * ✅ Added: In-App Notification/Reminder System (Slide Panel)
 * ✅ Keep: All existing features + Command Center integration
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    const CONFIG = {
        ...
        // ✅ NEW: Equipment list with Sound Portable & Display
        PERALATAN_LIST: [
    ...
    function decimalToTime(decimal) {
        ...
        const m = Math.round((decimal - h) * 60);        return `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}`;
    }
    ...
    async function checkDoubleBooking(data) {
        ...
        try {            const { data: existing, error } = await supabase
                .from('bookings')
                .select('*')
    ...
    async function loadNotifications() {
        ...
        try {
            ...
            return bookings?.map(b => ({
                ...b,
                isToday: b.tanggal === today,
                isTomorrow: b.tanggal === tomorrowStr
            })) || [];
        } catch (err) {
    ...
    function showNotificationPanel(notifications) {
        ...
        panel.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
                <h3 style="color:#3b82f6; font-size:1.1rem; font-weight:700;">
                    🔔 Reminder Booking
                </h3>
                <button onclick="document.getElementById('booking-notification-panel').remove()" style="background:none; border:none; color:#94a3b8; cursor:pointer; font-size:1.2rem;">
                    ✕
                </button>            </div>
            ...
            ${todayBookings.length > 0 ? `
                <div style="margin-bottom:1rem;">
                    <div style="background:rgba(16,185,129,0.1); border-left:3px solid #10b981; padding:0.75rem; border-radius:8px; margin-bottom:0.5rem;">
                        <div style="color:#10b981; font-weight:700; font-size:0.85rem; margin-bottom:0.25rem;">
                            📅 HARI INI (${todayBookings.length} booking)
                        </div>
                        ${todayBookings.map(b => `
            ...
            ${tomorrowBookings.length > 0 ? `
                <div>
                    <div style="background:rgba(245,158,11,0.1); border-left:3px solid #f59e0b; padding:0.75rem; border-radius:8px;">
                        <div style="color:#f59e0b; font-weight:700; font-size:0.85rem; margin-bottom:0.25rem;">
                            ⏰ BESOK (${tomorrowBookings.length} booking)
                        </div>
        ...
        setTimeout(() => {
            if (panel && panel.parentNode) {
                panel.style.transition = 'opacity 0.5s, transform 0.5s';                panel.style.opacity = '0';
    ...
    return `
        <div class="max-w-4xl mx-auto p-4">
            ...
            <div style="position:fixed; top:20px; right:20px; z-index:999;">
                <button id="booking-notif-btn" onclick="window.showBookingNotifications()" style="
            ...
            <div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem">
                ...
                </button>                <h2 class="text-2xl font-bold text-white">📅 Form Booking Sarana</h2>
            </div>
            ...
            <form id="bookingForm" class="space-y-6 bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                ...
                <!-- ✅ UPDATED: Peralatan dengan Sound Portable & Display -->
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 bg-slate-900/50 p-4 rounded-xl">
                        ${CONFIG.PERALATAN_LIST.map(alat => `
                            <label class="flex items-center gap-2 cursor-pointer hover:bg-slate-800 p-2 rounded-lg transition">
                                <input type="checkbox" name="alat" value="${alat}" class="w-4 h-4 rounded border-slate-600 text-emerald-500 focus:ring-emerald-500">                                <span class="text-sm text-slate-300">${alat}</span>
                            </label>
                        `).join('')}
                    </div>
                </div>
    ...
    setTimeout(async () => {
        ...
        // ✅ Load notifications on module init
        ...
        if (notifications.length > 0) {
            ...
            setTimeout(() => showNotificationPanel(notifications), 500);
        } else {
            if (notifBadge) notifBadge.style.display = 'none';
        }
        // Global function for notification button
        window.showBookingNotifications = async () => {
        ...
        form?.addEventListener('submit', async (e) => {
            ...
            doToast('🔍 Mengecek ketersediaan...', 'info');            submitBtn.disabled = true;
...
if (typeof window !== 'undefined') {
    console.log('📅 Booking module loaded | Sound Portable ✅ | In-App Notif ✅ | CC Integration: ON ✅');
}
