/**
 * Dream OS v2.0 - Booking Module (FIXED SIGNATURE)
 * Compatible with index.html module loader
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    async function checkDoubleBooking(data) {
        if (!supabase) return { hasConflict: false }; // Offline mode
        try {
            ...
            <form id="bookingForm" class="space-y-6 bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="time" name="jam_selesai" id="jam_selesai" required min="07:30" max="16:00" value="10:00" class="w-full p-3 rounded-xl bg-slate-900 border border-slate-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition text-white">                    </div>
                </div>
                <div>
    ...
    setTimeout(() => {
        ...
        tglInput?.addEventListener('change', (e) => {
            ...
            if (isSunday(dateStr) || isHoliday(dateStr)) {
                ...
                dateWarning.className = 'text-xs text-red-400 mt-1';            } else if (isSaturday(dateStr)) {
                dateWarning.classList.remove('hidden');
        ...
        form?.addEventListener('submit', async (e) => {
            ...
            const data = {
                nama: formData.get('nama'), divisi: formData.get('divisi'), no_hp: formData.get('no_hp'),
                sarana: formData.get('sarana'), tgl: formData.get('tgl'), jam_mulai: formData.get('jam_mulai'),
                jam_selesai: formData.get('jam_selesai'), peralatan: alatList.join(', '), keperluan: formData.get('keperluan')
            };

            const validation = validateBooking(data);
            if (validation.warnings.length) validation.warnings.forEach(w => utils.showToast(w, 'warning'));
            if (validation.errors.length) { validation.errors.forEach(err => utils.showToast(err, 'error')); return; }

            utils.showToast('🔍 Mengecek ketersediaan...', 'info');
            submitBtn.disabled = true; submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Checking...';

            const doubleCheck = await checkDoubleBooking(data);
            if (doubleCheck.hasConflict) {
                utils.showToast(doubleCheck.message, 'error');
            ...
            try {
                ...
                const payload = {
                    nama_peminjam: data.nama, divisi: data.divisi || null, no_hp: data.no_hp,
                    ruang: data.sarana, tanggal: data.tgl, jam_mulai: data.jam_mulai, jam_selesai: data.jam_selesai,
                    keperluan: data.keperluan || null, peralatan: data.peralatan, status: 'pending',
                ...
                utils.showToast('✅ Booking berhasil! Menunggu approval.', 'success');
                form.reset(); tglInput.value = getMinDate(); dateWarning.classList.add('hidden');
            } catch (err) {
...
if (typeof window !== 'undefined') {
    console.log('📅 Booking module loaded | Signature: MATCH ✅');
}
