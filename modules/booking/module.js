...
export default async function initModule(
    ...
    function renderNotifPanel(notifs) {
        ...
        panel.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.875rem">
                ...
                <button onclick="this.closest('#bk-notif-panel').remove()" style="background:none;border:none;color:#64748b;cursor:pointer;font-size:1rem">✕</button>
            </div>
    ...
    document.getElementById('bk-form')?.addEventListener('submit', async (e) => {
        ...
        if (dblCheck.conflict) {
            ...
            document.getElementById('bk-avail').innerHTML = `<span style="color:#ef4444">⛔ ${dblCheck.message}</span>`;
            return;
        }
        ...
        try {
            ...
            await writeAudit('Booking Baru', `${data.sarana} · ${data.tgl} · ${data.jam_mulai}–${data.jam_selesai} · ${data.nama}`);

            toast(`✅ Booking ${data.sarana} berhasil! ${data.tgl} ${data.jam_mulai}–${data.jam_selesai}`, 'success');
            document.getElementById('bk-avail').innerHTML = `<span style="color:#10b981">✅ Booking berhasil disimpan!</span>`;
