// modules/commandcenter/module.js - VERSI FINAL
...
console.log('[COMMANDCENTER] Final version loaded');
...
function renderHTML() {
    ...
    container.innerHTML = `
        ...
        <!-- CONTENT AREA -->
        <div id="content-area" class="content-area glass-card p-4"></div>
...
async function loadAllStats() {
    ...
    try {
        ...
        console.error('[COMMANDCENTER] Stats error:', err);
    }
}
...
function renderSubmenu(tab) {
    ...
    const modulesByTab = {
        ...
        approval: [
            { id: 'approval-booking', name: 'Booking', icon: '📅' },
            { id: 'approval-k3', name: 'K3', icon: '⚠️' },
            { id: 'approval-dana', name: 'Dana', icon: '💰' }
        ],
        files: [
            { id: 'files-k3', name: 'K3 Foto', icon: '🖼️' },
            { id: 'files-spj', name: 'SPJ Foto', icon: '📸' }
        ],
        backup: [
            { id: 'backup-create', name: 'Create', icon: '💾' },
            { id: 'backup-restore', name: 'Restore', icon: '🔄' }
        ]
    };
...
async function loadRecentBookings() {
    ...
    if (!container) return;
    try {
        const data = await api.query('bookings', { status: 'pending' }, { order: 'created_at', ascending: false, limit: 5 });
        if (!data || data.length === 0) {
        ...
        container.innerHTML = data.map(item => `
            <div class="glass-card p-2 flex justify-between items-center">
                ...
                <div class="flex gap-1">
                    <button onclick="handleApprove('${item.id}', 'bookings')" class="bg-emerald-600 hover:bg-emerald-500 px-2 py-1 rounded text-xs">✓</button>
                    <button onclick="handleReject('${item.id}', 'bookings')" class="bg-red-600 hover:bg-red-500 px-2 py-1 rounded text-xs">✗</button>
                </div>
            </div>
        `).join('');
    } catch (err) {
...
async function loadRecentK3() {
    ...
    if (!container) return;
    try {
        const data = await api.query('k3_reports', { status: 'pending' }, { order: 'created_at', ascending: false, limit: 5 });
        if (!data || data.length === 0) {
        ...
        container.innerHTML = data.map(item => `
            <div class="glass-card p-2 flex justify-between items-center">
                ...
                <div class="flex gap-1">
                    <button onclick="handleApprove('${item.id}', 'k3_reports')" class="bg-emerald-600 hover:bg-emerald-500 px-2 py-1 rounded text-xs">✓</button>
                    <button onclick="handleReject('${item.id}', 'k3_reports')" class="bg-red-600 hover:bg-red-500 px-2 py-1 rounded text-xs">✗</button>
                </div>
            </div>
        `).join('');
    } catch (err) {
...
async function loadRecentDana() {
    ...
    if (!container) return;
    try {
        const data = await api.query('pengajuan_dana', { status: 'pending' }, { order: 'created_at', ascending: false, limit: 5 });
        if (!data || data.length === 0) {
        ...
        container.innerHTML = data.map(item => `
            <div class="glass-card p-2 flex justify-between items-center">
                ...
                <div class="flex gap-1">
                    <button onclick="handleApprove('${item.id}', 'pengajuan_dana')" class="bg-emerald-600 hover:bg-emerald-500 px-2 py-1 rounded text-xs">✓</button>
                    <button onclick="handleReject('${item.id}', 'pengajuan_dana')" class="bg-red-600 hover:bg-red-500 px-2 py-1 rounded text-xs">✗</button>
                </div>
            </div>
        `).join('');
    } catch (err) {
...
async function updateStatus(table, id, status) {
    ...
    try {
        await api.update(table, id, { status, updated_at: new Date() });
