...
// Dream OS v2.0 — Sekuriti Module with Interactive Shift Scheduler
// Fitur: Laporan patroli, riwayat, jadwal piket interaktif (manual + auto generate + drag & drop)
...
function injectCSS() {
    ...
    s.textContent = `
        ...
        .sek-panel { background: var(--sek-bg-panel); backdrop-filter: blur(18px); border: 1px solid var(--sek-primary-border); border-radius: var(--sek-radius); padding: 1.5rem; margin-bottom: 1.5rem; }
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang, container) {
    if (!container) {
        console.error('[SEKURITI] Container tidak diberikan');
        return;
    }
    ...
    let _tab = 'laporan'; // laporan, history, jadwal, harian
    ...
    function render() {
        ...
        <div id="sekuriti-root">
            ...
            <div class="sek-tabs">
                <button class="sek-tab ${_tab === 'laporan' ? 'active' : ''}" data-tab="laporan">📋 Laporan Patroli</button>
                <button class="sek-tab ${_tab === 'history' ? 'active' : ''}" data-tab="history">📜 Riwayat</button>
                <button class="sek-tab ${_tab === 'jadwal' ? 'active' : ''}" data-tab="jadwal">📅 Jadwal Piket</button>
                <button class="sek-tab ${_tab === 'harian' ? 'active' : ''}" data-tab="harian">📊 Laporan Harian</button>
            </div>

            <div id="sek-laporan-tab" class="tab-content" style="display: ${_tab === 'laporan' ? 'block' : 'none'};">
                ${renderLaporanForm()}
            </div>
            <div id="sek-history-tab" class="tab-content" style="display: ${_tab === 'history' ? 'block' : 'none'};">
                ${renderHistory()}
            </div>
            <div id="sek-jadwal-tab" class="tab-content" style="display: ${_tab === 'jadwal' ? 'block' : 'none'};">
                ${renderJadwal()}
            </div>
            <div id="sek-harian-tab" class="tab-content" style="display: ${_tab === 'harian' ? 'block' : 'none'};">
                ${renderHarian()}
            </div>
        </div>
        `;
        ...
        container.querySelectorAll('.sek-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (_tab === 'jadwal') loadJadwal();
                if (_tab === 'harian') loadHarian();
            });
        });

        // Event listener khusus per tab
        if (_tab === 'laporan') attachLaporanEvents(container);
        if (_tab === 'history') attachHistoryEvents(container);
        if (_tab === 'jadwal') attachJadwalEvents(container);
        if (_tab === 'harian') attachHarianEvents(container);

        // Update lokasi berkala
        updateLocationStatus(container);
        setInterval(() => updateLocationStatus(container), 30000);
    }
        ...
        <div class="sek-panel">
            ...
            <div class="sek-table-wrap" style="max-height:500px; overflow-y:auto;">
                <table class="sek-table sek-schedule-table" id="sek-schedule-table">
                    <thead id="sek-schedule-header">
                        <tr><th>Petugas</th></tr>
                    </thead>
                    <tbody id="sek-schedule-body">
                        <!-- akan diisi JS -->
                    </tbody>
                </table>
            </div>
    ...
    function attachLaporanEvents(container) {
    ...
    function attachHistoryEvents(container) {
        ...
        // Detail button delegation di loadHistory
    }

    function attachJadwalEvents(container) {
        // Load jadwal saat tab dibuka
        loadJadwal(container);

        container.querySelector('#sek-save-template').addEventListener('click', () => saveTemplate(container));
        container.querySelector('#sek-load-template').addEventListener('click', () => loadTemplate(container));
        container.querySelector('#sek-generate-auto').addEventListener('click', () => generateAutoJadwal(container));
        container.querySelector('#sek-save-jadwal').addEventListener('click', () => saveJadwal(container));
    }

    function attachHarianEvents(container) {
        loadHarian(container);
        container.querySelector('#export-harian-pdf')?.addEventListener('click', () => exportHarianPDF(container));
        container.querySelector('#export-harian-excel')?.addEventListener('click', () => exportHarianExcel(container));
    }

    // === Logika Jadwal Interaktif ===
    async function loadJadwal(container) {
        ...
        // Aktifkan SortableJS untuk drag & drop baris
        if (window.Sortable) {
            new Sortable(tbody, {
                handle: '.sek-drag-handle',
                animation: 150,
                onEnd: function(evt) {
                    // Urutan baris berubah, kita bisa update data jika perlu
                    toast('Urutan petugas berubah', 'info');
                }
            });
        } else {
            console.warn('SortableJS tidak tersedia, drag & drop nonaktif');
        }

        // Event change pada select
        tbody.querySelectorAll('.sek-schedule-input').forEach(sel => {
            sel.addEventListener('change', function(e) {
                // Opsional: auto-save via debounce
                const nama = this.dataset.nama;
                const tgl = this.dataset.tgl;
                const val = this.value;
                console.log(`[Jadwal] ${nama} tgl ${tgl} -> ${val}`);
    ...
    async function saveJadwal(container) {
        ...
        try {
            ...
            for (const item of dataToSave) {
                await _sb
                    .from('sekuriti_jadwal_master')
                    .upsert(item, { onConflict: 'petugas_name, bulan, tahun' });
            }
    ...
    function generateAutoJadwal(container) {
        const bulan = new Date().getMonth() + 1;
        const tahun = new Date().getFullYear();
        const jmlHari = new Date(tahun, bulan, 0).getDate();
        ...
        // Pola sederhana: 3 hari kerja, 2 hari libur, bergilir
        // Hari ke-1: Petugas 1-3 P, 4-5 M, 6-7 L
        // Setiap hari shift berputar
        for (let tgl = 1; tgl <= jmlHari; tgl++) {
            const offset = Math.floor((tgl - 1) / 5) % 7; // cycle 5 hari
            for (let i = 0; i < petugas.length; i++) {
                const idx = (i + offset) % petugas.length;
                if (!jadwalBaru[petugas[i]]) jadwalBaru[petugas[i]] = [];
                if (idx < 3) jadwalBaru[petugas[i]][tgl-1] = 'P';
                else if (idx < 5) jadwalBaru[petugas[i]][tgl-1] = 'M';
                else jadwalBaru[petugas[i]][tgl-1] = 'L';
            }
        }

        // Koreksi khusus Donih: tidak boleh M
        const donihIndex = petugas.indexOf('DONIH');
        if (donihIndex !== -1) {
            for (let tgl = 1; tgl <= jmlHari; tgl++) {
                if (jadwalBaru['DONIH'][tgl-1] === 'M') {
                    // Cari petugas lain yang libur hari itu untuk ditukar
        ...
        tbody.querySelectorAll('tr').forEach(row => {
            ...
            selects.forEach((sel, idx) => {
                sel.value = jadwalBaru[nama][idx] || 'L';
            });
        });
    ...
    function saveTemplate(container) {
        ...
        localStorage.setItem('sekuriti_template', json);
    ...
    function loadTemplate(container) {
        ...
        if (!json) {
            toast('Tidak ada template tersimpan', 'warning');
            return;
        }
        try {
            ...
            tbody.querySelectorAll('tr').forEach(row => {
                ...
                if (templateItem) {
                    const selects = row.querySelectorAll('.sek-schedule-input');
                    selects.forEach((sel, idx) => {
                        if (templateItem.shifts[idx]) sel.value = templateItem.shifts[idx];
                    });
                }
            });
            toast('✅ Template dimuat', 'success');
        } catch (err) {
            toast('❌ Template rusak', 'error');
        }
    }

    // === Logika Laporan Harian ===
    async function loadHarian(container) {
        ...
        let html = `
            <p class="text-sm text-slate-400 mb-2">Tanggal: ${today}</p>
        `;
        if (!data || data.length === 0) {
            ...
            data.forEach(item => {
                ...
                html += `<tr>
                    <td>${waktu}</td>
                    <td>${Array.isArray(item.petugas) ? item.petugas.join(', ') : item.petugas}</td>
    ...
    function exportHarianPDF(container) {
        if (!window.jspdf?.jsPDF) {
            toast('jsPDF tidak tersedia', 'error');
            return;
        }
        // Implementasi ekspor PDF (ringkas)
        alert('Fitur ekspor PDF akan segera hadir!');
    }

    function exportHarianExcel(container) {
        if (!window.XLSX) {
            toast('SheetJS tidak tersedia', 'error');
            return;
        }
        // Implementasi ekspor Excel (ringkas)
        alert('Fitur ekspor Excel akan segera hadir!');
    }

    // === Logika Laporan Patroli (Submit) ===
    async function handleSubmit(e) {
        ...
        try {
            ...
            form.reset();
            container.querySelector('#sek-preview').style.display = 'none';
            container.querySelector('#sek-foto-base64').value = '';
            container.querySelector('#sek-tanggal').value = new Date().toISOString().split('T')[0];
            container.querySelector('#sek-shift-input').value = getCurrentShift();
        } catch (err) {
            console.error('[SEKURITI] Submit error:', err);
            toast('Gagal: ' + err.message, 'error');
            container.querySelector('#sek-form-result').innerHTML = `<span style="color:#ef4444;">❌ ${err.message}</span>`;
        } finally {
            btn.disabled = false;
    ...
    async function loadHistory() {
        ...
        try {
            ...
            tbody.querySelectorAll('[data-action="detail"]').forEach(btn => {
                btn.addEventListener('click', () => {
                    toast('Detail laporan ID: ' + btn.dataset.id, 'info');
                });
            });
        } catch (err) {
    ...
    // === Update lokasi GPS ===
    async function updateLocationStatus(container) {
        try {
            const coords = await getGeolocation();
            const { safe, distance } = checkSafeCore(coords.latitude, coords.longitude);
            container.querySelector('#sek-lokasi').textContent = `${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)}`;
            container.querySelector('#sek-jarak').textContent = `${distance.toFixed(1)} km`;
            container.querySelector('#sek-status').innerHTML = safe
                ? '<span class="sek-badge-aman">AMAN</span>'
                : '<span class="sek-badge-danger">LUAR CORE</span>';
        } catch (err) {
            container.querySelector('#sek-lokasi').textContent = 'Tidak dapat diakses';
            container.querySelector('#sek-jarak').textContent = '-';
            container.querySelector('#sek-status').innerHTML = '<span class="sek-badge-danger">GPS ERROR</span>';
        }
    }
