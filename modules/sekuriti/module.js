 ...
 * Dream OS v2.0 — Sekuriti Module (FINAL)
 * ✅ Fitur: Laporan patroli dengan GPS & foto, riwayat, jadwal interaktif, laporan harian
...
/* ==========================================================================
   CSS INJECTION (dengan variabel lengkap)
   ========================================================================== */
function injectCSS() {
    ...
    s.textContent = `
        :root {
            ...
            --sek-border: rgba(255,255,255,0.08); /* ✅ sudah ada */
        }
        ...
        .sek-sub { font-size: 0.75rem; color: var(--sek-text-muted); }
...
/* ==========================================================================
   EXPORT DEFAULT — Mengembalikan HTML shell, lalu inisialisasi async
   ========================================================================== */
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang, container) {
    // ✅ Langsung kembalikan HTML shell agar parent bisa render
    const shellHTML = buildShell(currentUser);
    
    // Inisialisasi async setelah parent memasukkan HTML
    setTimeout(async () => {
        if (!container) {
...
/* ==========================================================================
   MEMBANGUN HTML SHELL (dikembalikan ke parent)
   ========================================================================== */
        ...
        <div class="sek-panel sek-header">
            <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
                ...
                <div style="margin-left:auto; display:flex; gap:0.5rem;">
                    <span style="background:rgba(139,92,246,0.15);border:1px solid rgba(139,92,246,0.3);color:#a855f7;padding:0.4rem 1rem;border-radius:30px;font-size:0.8rem;">${escapeHTML(userName)}</span>
                </div>
            </div>
        </div>

        <div class="sek-status-grid">
            <div class="sek-status-card"><div class="sek-status-label">SHIFT</div><div class="sek-status-value" id="sek-shift">—</div></div>
            <div class="sek-status-card"><div class="sek-status-label">DATABASE</div><div class="sek-status-value" id="sek-db-status">—</div></div>
            <div class="sek-status-card"><div class="sek-status-label">LOKASI</div><div class="sek-status-value" id="sek-lokasi">—</div></div>
            <div class="sek-status-card"><div class="sek-status-label">JARAK</div><div class="sek-status-value" id="sek-jarak">—</div></div>
        </div>
    ...
    <div class="sek-panel">
        ...
        <div class="sek-table-wrap" style="max-height:500px; overflow-y:auto;">
            <table class="sek-table sek-schedule-table" id="sek-schedule-table">
                ...
                <tbody id="sek-schedule-body"></tbody>
            </table>
        </div>
...
/* ==========================================================================
   LOGIKA MODUL (async, setelah shell di-inject)
   ========================================================================== */
async function initModuleLogic(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang, container) {
    ...
    const esc = utils?.esc || escapeHTML;
    ...
    // ===== DATABASE CONNECTION =====
    // Gunakan shared client dari window.S jika ada (dari main app)
    if (window.S?.sb) {
        _sb = window.S.sb;
        console.log('[SEKURITI] ✅ Menggunakan shared Supabase client');
    }

    async function ensureSB() {
        if (_sb) {
            try {
                await _sb.from('audit_logs').select('count', { count: 'exact', head: true });
                console.log('[SEKURITI] ✅ Koneksi database terverifikasi');
                return true;
            } catch (e) {
                console.warn('[SEKURITI] ⚠️ Test koneksi gagal, coba inisialisasi ulang:', e.message);
                _sb = null;
            }
        }
        ...
        if (window.supabase?.createClient) {
            try {
                ...
                console.log('[SEKURITI] ✅ Supabase diinisialisasi via window.supabase');
                return true;
            } catch (e) {
                console.error('[SEKURITI] ❌ Gagal inisialisasi via window.supabase:', e.message);
            }
        }
        ...
        return new Promise((resolve) => {
            ...
            script.onload = () => {
                if (window.supabase?.createClient) {
                    _sb = window.supabase.createClient(SB_URL_FALLBACK, SB_KEY_FALLBACK);
                    console.log('[SEKURITI] ✅ Supabase loaded via CDN');
                    resolve(true);
                } else {
                    console.error('[SEKURITI] ❌ CDN loaded tapi createClient tidak tersedia');
                    resolve(false);
                }
            };
            script.onerror = () => {
                console.error('[SEKURITI] ❌ Gagal load CDN Supabase');
                resolve(false);
            };
    ...
    async function updateStatusBar() {
        ...
        const dbStatusEl = container.querySelector('#sek-db-status');
        if (shiftEl) shiftEl.textContent = getCurrentShift();

        if (!_sb) {
            if (dbStatusEl) {
                dbStatusEl.innerHTML = '<span class="sek-badge-danger">OFFLINE</span>';
                dbStatusEl.title = 'Tidak terhubung ke database';
            }
            return;
        }

        try {
            await _sb.from('audit_logs').select('id', { limit: 1 });
            if (dbStatusEl) {
                dbStatusEl.innerHTML = '<span class="sek-badge-aman">ONLINE</span>';
                dbStatusEl.title = 'Database terhubung';
            }
        } catch (e) {
            if (dbStatusEl) {
                dbStatusEl.innerHTML = '<span class="sek-badge-warning">LIMITED</span>';
                dbStatusEl.title = 'Mode terbatas (hanya baca)';
            }
        }
    }
    ...
    async function loadHarian() {
        ...
        await ensureSB();
        if (!_sb) {
            contentDiv.innerHTML = '<p class="text-center py-4 text-slate-500">Database offline, tidak dapat memuat laporan harian.</p>';
            return;
        }
    ...
    async function loadHistory() {
        ...
        if (!tbody || !_sb) {
            if (tbody) tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2rem;">Database offline</td></tr>';
            return;
        }
    ...
    async function loadJadwal() {
        ...
        let data = [];
        if (_sb) {
            const res = await _sb.from('sekuriti_jadwal_master').select('petugas_name, jadwal_array').eq('bulan', bulan).eq('tahun', tahun);
            data = res.data || [];
        }
        ...
        listPetugas.forEach((nama, idx) => {
            ...
            html += `<td style="display:flex; align-items:center;"><span class="sek-drag-handle" draggable="true">☰</span> ${nama}</td>`;
    ...
    async function saveJadwal() {
        ...
        if (!_sb) {
            toast('Database offline, simpan ke localStorage sebagai template', 'warning');
            saveTemplate(); // fallback simpan template
            return;
        }
    ...
    async function handleSubmit(e) {
        ...
        // Offline mode
        if (!_sb) {
            const offlineReports = JSON.parse(localStorage.getItem('sekuriti_offline') || '[]');
            offlineReports.push({
                tanggal, shift, petugas, lokasi, deskripsi,
                foto_base64: fotoBase64, // simpan base64 (hati-hati besar)
                created_at: new Date().toISOString(),
                status: 'pending',
                offline: true
            });
            localStorage.setItem('sekuriti_offline', JSON.stringify(offlineReports));
            toast('⚠️ Offline mode — laporan disimpan lokal', 'warning');
            form.reset();
    ...
    async function syncOfflineReports() {
        if (!_sb) return;
        const offline = JSON.parse(localStorage.getItem('sekuriti_offline') || '[]');
        if (!offline.length) return;

        toast(`🔄 Sinkronisasi ${offline.length} laporan offline...`, 'info');
        let sukses = 0;
        for (const report of offline) {
            try {
                // Upload foto dulu jika perlu (dari base64)
                let fotoUrl = null;
                if (report.foto_base64) {
                    fotoUrl = await uploadPhoto(report.foto_base64);
                }
                const { foto_base64, offline, ...data } = report;
                if (fotoUrl) data.foto_url = [fotoUrl];
                await _sb.from('sekuriti_reports').insert([data]);
                sukses++;
            } catch (e) {
                console.warn('[SEKURITI] Gagal sinkron:', e.message);
            }
        }

        const remaining = offline.slice(sukses);
        localStorage.setItem('sekuriti_offline', JSON.stringify(remaining));
        toast(`✅ ${sukses}/${offline.length} laporan tersinkron`, sukses === offline.length ? 'success' : 'warning');
    }
    ...
    // ===== INIT =====
    await ensureSB();
    if (_sb) await syncOfflineReports();
    await updateStatusBar();
    bindEvents();
    updateLocationStatus();
    setInterval(updateLocationStatus, 30000);
    setInterval(updateStatusBar, 60000); // update shift dan status

    console.log('[SEKURITI] ✅ Siap — Bi idznillah 💚');
}