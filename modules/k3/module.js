...
console.log('[K3] Module initialized');
    ...
    container.innerHTML = `
        <div class="max-w-6xl mx-auto p-4">
            ...
            <div class="bg-gradient-to-r from-slate-800 to-slate-900 p-6 rounded-2xl mb-6 border border-orange-500/30">
                <h2 class="text-2xl font-bold text-orange-400">⚠️ K3 REPORT</h2>
            ...
            <form id="k3Form" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm mb-1" id="label-date">${TRANSLATIONS[currentLang].date}</label>
                    ...
                    <div>
                        <label class="block text-sm mb-1" id="label-location">${TRANSLATIONS[currentLang].location}</label>
                ...
                <div>
                    <label class="block text-sm mb-1" id="label-type">${TRANSLATIONS[currentLang].reportType}</label>
                ...
                <div>
                    <label class="block text-sm mb-1" id="label-description">${TRANSLATIONS[currentLang].description}</label>
                ...
                <div>
                    <label class="block text-sm mb-1" id="label-photo">${TRANSLATIONS[currentLang].photo}</label>
                    <div class="border-2 border-dashed border-orange-500/50 rounded-lg p-4 text-center cursor-pointer hover:bg-orange-500/10 transition" onclick="document.getElementById('cameraInput').click()">
                        ...
                        <p class="text-sm" id="photo-instruction">${TRANSLATIONS[currentLang].photoInstruction}</p>
                        <p class="text-xs opacity-60" id="photo-max">${TRANSLATIONS[currentLang].photoMax}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm mb-1" id="label-reporter">${TRANSLATIONS[currentLang].reporter}</label>
                    ...
                    <div>
                        <label class="block text-sm mb-1" id="label-priority">${TRANSLATIONS[currentLang].priority}</label>
...
export async function init() {
    ...
    cameraInput?.addEventListener('change', (e) => {
        ...
        if (file) {
            ...
            reader.onload = (event) => {
                ...
                fotoBase64.value = event.target.result; // Simpan base64
            };
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + TRANSLATIONS[currentLang].submitting;
        ...
        try {
            ...
            if (fotoBase64.value) {
                ...
                if (photoUrl) {
                    // Simpan sebagai array dengan satu elemen (sesuai tipe kolom text[])
                    data.foto_url = [photoUrl];
                } else {
                    // Jika upload gagal, simpan base64 sebagai string biasa (ubah kolom jadi text? Tapi lebih baik tetap array dengan base64)
                    // Alternatif: simpan base64 sebagai array
            ...
            btn.disabled = false;
            btn.innerHTML = TRANSLATIONS[currentLang].submit;
        }
    });
}
...
async function uploadPhoto(base64Data) {
    try {
        ...
        return null; // Gagal upload, kembalikan null
    }
}
