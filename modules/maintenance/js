/**
 * Maintenance Module - Professional System 2026
 * Dream OS v2.0 | The Power Soul of Shalawat
 * 
 * Features:
 * - QR Code Scanner (Asset tracking)
 * - Camera Before/After (Documentation)
 * - Maintenance History (Timeline)
 * - Gudang Integration (Sparepart)
 * - Inventory Integration (Usage)
 * - Command Center Sync (Real-time)
...
const TRANSLATIONS = {
    id: {
        ...
        scanQR: 'Scan QR Asset',
        createTask: 'Buat Tugas',
        cancel: 'Batal',        takeTask: 'Ambil Tugas',
        sparepart: 'Sparepart',
        update: 'Update',
        finish: 'Selesai',
        detail: 'Detail',
...
export function init() {
    console.log('[MAINTENANCE] Module initialized - Professional Edition');
        // Apply language
...
function applyLanguage(lang) {
    ...
    // Update text content
    const elements = {
        'maintenance-title': t.title,
        'btn-new-task': t.newTask,
        'stat-total-label': t.total,
        'stat-pending-label': t.pending,
        'stat-proses-label': t.proses,
        'stat-selesai-label': t.selesai
    };
...
function setupEventListeners() {
    ...
    document.getElementById('sparepart-batal')?.addEventListener('click', closeSparepartModal);
}
...
// ===== LOAD TASKS =====async function loadTasks(filter = 'semua') {
    const container = document.getElementById('tasks-list');
    const t = TRANSLATIONS[currentLang];
    
    container.innerHTML = '<div class="text-center py-12"><div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-orange-500 border-t-transparent"></div><p class="mt-2 opacity-60">' + t.loading + '</p></div>';
    ...
    try {
        ...
        if (!data || data.length === 0) {
            container.innerHTML = '<p class="text-center py-12 opacity-60">' + t.noTasks + '</p>';
            return;
        }
...
window.bukaModalSparepart = async (taskId) => {
    ...
    stokList = data || [];    const select = document.getElementById('sparepart-barang');
...
window.closeSparepartModal = function() {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
            .then(function(stream) {
                ...
                // Simulate scan (in production, use jsQR library)
                setTimeout(() => {
...
async function loadHistory() {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p class="text-center opacity-60">' + t.noHistory + '</p>';
        return;
    }
...
window.viewTaskDetail = async function(taskId) {
    showToast('Viewing task detail...', 'success');
    // In production, open detail modal with full task info
};
...
// Auto-initif (document.readyState === 'loading') {
