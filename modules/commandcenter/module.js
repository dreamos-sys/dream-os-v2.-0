...
const modulesByTab = {
  ...
  dana: [
    ...
    { id:'approval', name:'Approval', icon:'✅' },
    { id:'qr', name:'QR', icon:'📱' },    { id:'backup', name:'Backup', icon:'💾' },
    { id:'laporan', name:'Laporan', icon:'📄' },
    { id:'analytics', name:'Analytics', icon:'📈' }
  ]
};
...
function generateAIInsights(c) {
  ...
  if (pl) {
    pl.innerHTML = [
      ...
      { cls: c.k3>3 ? 'high':'normal', ic:'fa-exclamation-circle', color: c.k3>3?'var(--or)':'var(--em)', txt: c.k3>3?`⚠️ ${c.k3} K3 reports butuh perhatian`:'✅ K3 dalam batas aman' },      { cls: c.stok>0 ? 'critical':'normal', ic:'fa-exclamation-triangle', color: c.stok>0?'var(--re)':'var(--em)', txt: c.stok>0?`🔴 ${c.stok} item stok kritis`:'✅ Stok dalam batas aman' }
    ].map(p => `
  ...
  if (tab === 'dashboard') {
    c.innerHTML = `
      ...
      <div style="margin-top:1rem">
        ...
        <div class="health-item"><div class="health-header"><span>Database</span><span>98%</span></div><div class="health-bar"><div class="health-fill green" style="width:98%"></div></div></div>        <div class="health-item"><div class="health-header"><span>API</span><span>100%</span></div><div class="health-bar"><div class="health-fill green" style="width:100%"></div></div></div>
...
// ── WINDOW GLOBALS (NAVIGATION) ──────────────────────
// FIX: NO DUPLICATE - Only define if not already exists
if (!window.openSubModule) {
...
if (!window.createBackup) {
  window.createBackup = async function() {
    ...
    a.click();
    URL.revokeObjectURL(a.href);    showToast('✅ Backup berhasil!', 'success');
  };
}
...
console.log('[COMMANDCENTER] ✅ Ready - No duplicates');
