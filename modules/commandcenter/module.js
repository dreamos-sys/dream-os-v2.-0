...
import { predictStock, getAIRecommendations } from '../../core/ai.js'; // jika ada
...
async function loadAllData() {
    try {
        // Ambil data pending dari semua modul
        const [booking, k3, dana, stok, maintenance, janitorIn, janitorOut, sekuriti, toolRequests, assets, gudang] = await Promise.all([
            ...
            supabase.from('inventory').select('*').lt('jumlah', 'minimal_stok'), // stok menipis
...
function generateAIInsights(counts) {
    ...
    // Predictive items (bisa dari AI engine)
    const predictions = [
