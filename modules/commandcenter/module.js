 ...
 * ✅ FIXED: Signature match + shared Supabase + integrated with Booking & K3 modules
...
/* ══════════════════════════════════════════════════════════
   CONSTANTS
   ✅ SB_URL/SB_KEY dihapus dari sini — Supabase diterima dari loader
      (sama seperti booking & K3 module)
══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════
   EXPORT DEFAULT — ✅ Signature seragam dengan booking & K3
   Parameter: (config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang)
══════════════════════════════════════════════════════════ */
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    // ✅ Unified toast — sama polanya dengan booking & K3 module
    function doToast(msg, type) {
        type = type || 'success';
        if (utils?.showToast)               return utils.showToast(msg, type);
        if (typeof showToast === 'function') return showToast(msg, type);
        // fallback built-in (untuk standalone mode)
    ...
    // ════════════════════════════════════════
    // MOUNT LOGIC — setTimeout(100ms) agar DOM siap
    // ✅ Sama polanya dengan booking & K3 module
    // ════════════════════════════════════════
    setTimeout(() => {
        /* ── state ── */
        let _sb       = supabase || null;   // ✅ Pakai supabase dari loader, bukan buat sendiri
        ...
        // ✅ AUDIT LOG WRITER — dipakai oleh approval & operasi lain
        //    Booking & K3 juga menulis ke tabel yang sama, sehingga
        //    tab Activity Command Center menampilkan aktivitas keduanya
        ...
        /* ── fallback: init supabase jika loader tidak menyediakan ── */
        ...
        const clkTimer = setInterval(() => {
            const el = getEl('cc2-clock');
            if (el) el.textContent = new Date().toLocaleTimeString('id-ID');
        }, 1000);
        _timers.push(clkTimer);

        /* ── LOAD STATS ─────────────────────────────────────────────
           Membaca bookings + k3_reports dari tabel yang sama
           dengan booking module & k3 module → angka selalu sinkron
        ─────────────────────────────────────────────────────────── */
        ...
        async function loadActivityFeed() {
            ...
            // ✅ Membaca audit_logs — yang ditulis oleh booking module, k3 module, DAN command center sendiri
        ...
        async function loadPendingQueue() {
            ...
            // ✅ Membaca dari tabel yang sama persis dengan booking module & k3 module
        ...
        async function loadFullLog() {
            ...
            // ✅ Membaca audit_logs yang ditulis oleh SEMUA module: booking, K3, dan command center sendiri
        ...
        async function handleApprove(tbl, id, status) {
            ...
            // ✅ Tulis ke audit_logs — akan muncul di Activity feed
            await writeAuditLog(
        ...
        function bindEvents() {
            ...
            getEl('cc2-qa-refresh')?.addEventListener('click', () => { doToast('🔄 Refreshing...','info'); loadStats(); });
        ...
        /* ── REALTIME SUBSCRIPTION ──────────────────────────────────────
           ✅ Listen ke INSERT pada bookings & k3_reports
              Saat booking module / k3 module berhasil insert → counter langsung update
        ──────────────────────────────────────────────────────────────── */
        function subscribeRealtime() {
            if (!_sb) return;
            try {
                _channel = _sb.channel('cc2-live')
                    .on('postgres_changes',{event:'INSERT',schema:'public',table:'bookings'},      () => { loadStats(); doToast('📅 Booking baru masuk!','info'); })
                    .on('postgres_changes',{event:'INSERT',schema:'public',table:'k3_reports'},    () => { loadStats(); doToast('⚠️ Laporan K3 baru!','warning'); })
                    .on('postgres_changes',{event:'INSERT',schema:'public',table:'pengajuan_dana'},() => { loadStats(); doToast('💰 Pengajuan dana baru!','info'); })
                    .on('postgres_changes',{event:'INSERT',schema:'public',table:'spj'},           () => { loadStats(); doToast('📋 SPJ baru masuk!','info'); })
                    .on('postgres_changes',{event:'INSERT',schema:'public',table:'audit_logs'},    () => {
                        // Refresh activity feed jika sedang di tab activity atau dashboard
        ...
        /* ── STORE CLEANUP pada window agar module loader bisa panggil ── */
        window._cc2_cleanup = function cleanup() {
            ...
            delete window._cc2_cleanup;
            delete window._cc2_detail;
        };

        /* ── MAIN INIT ── */
        async function init() {
            // ✅ Ambil user dari parameter loader (bukan dari DOM badge)
            if (currentUser) {
                _user = currentUser;
            ...
            const weatherTimer = setInterval(loadWeather, 20*60*1000);
            _timers.push(refreshTimer, weatherTimer);
        }

        init();

    }, 100); // ✅ Delay 100ms agar DOM siap — sama persis dengan booking & K3 module

    return buildShell(currentUser);
}

if (typeof window !== 'undefined') {
    console.log('🚀 Command Center module loaded | Signature: MATCH ✅ | Booking+K3 Integration: ON ✅');
}
