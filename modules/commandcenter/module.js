 ...
 * ✅ Fetch config dari /api/config
 * ✅ Debug panel untuk troubleshooting di HP
 * ✅ Semua fitur lengkap (dashboard, approval, dana, spj, activity, analytics, system)
 */

'use strict';

// ==================== DEBUG SETUP (tampil di HP) ====================
(function setupDebug() {
...
function dbg(msg, type = 'info') {
    _dbgCount++;
    const colors = { info: { bg: '#0f172a', border: '#10b981', text: '#10b981' }, warn: { bg: '#0f172a', border: '#f59e0b', text: '#f59e0b' }, error: { bg: '#0f172a', border: '#ef4444', text: '#ef4444' }, success: { bg: '#0f172a', border: '#3b82f6', text: '#3b82f6' } };
    ...
    setTimeout(() => { d.style.opacity = '0'; setTimeout(() => d.remove(), 400); }, 7000);
...
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    async function loadWeather() {
        try {
            const url = (env?.wkey) ? `/api/weather` : FALLBACK_WEATHER_URL;
