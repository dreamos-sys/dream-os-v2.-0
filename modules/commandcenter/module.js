 ...
 * ✅ Mengambil konfigurasi dari Cloudflare Functions (/api/config)
 * ✅ Manajemen timer anti-memory leak
...
/* ══════════════════════════════════════════════════════════
   CONSTANTS (hanya fallback, tidak digunakan jika fetch sukses)
══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════
   EXPORT DEFAULT — dengan fetch config dari Cloudflare
══════════════════════════════════════════════════════════ */
export default async function initModule(config, utils, supabase, currentUser, showToast, showModal, loader, translations, currentLang) {
    ...
    // ── STEP 1: Ambil konfigurasi dari Cloudflare Edge ─────────────────
    let env = null;
    try {
        ...
        if (res.ok) {
            ...
            console.warn('[CC] Gagal fetch config, menggunakan fallback (tidak aman!)');
        }
    } catch (e) {
        console.error('[CC] Fetch config error:', e.message);
    }

    // ── STEP 2: Fallback jika fetch gagal (hanya untuk development) ────
    if (!env || !env.url || env.url === 'UNDEFINED') {
        console.warn('[CC] Menggunakan fallback konfigurasi (tidak aman untuk production)');
        env = {
        ...
        if (showToast) showToast('Mode pengembangan: fallback config', 'warning');
    }

    // ── STEP 3: Inisialisasi Supabase client ─────────────────────────
    const sb = supabase.createClient(env.url, env.key);

    // ── STEP 4: Timer manager (anti memory leak) ─────────────────────
    ...
    // ── STEP 5: State lokal (sama seperti sebelumnya) ─────────────────
    let _sb       = sb;
    ...
    async function loadWeather() {
        let weatherUrl = FALLBACK_WEATHER_URL;
        if (env && env.wkey) {
            weatherUrl = `https://api.openweathermap.org/data/2.5/weather?q=${env.loc || 'Depok'}&appid=${env.wkey}&units=metric&lang=id`;
        }
        try {
            ...
            if (d.weather[0].main.toLowerCase().includes('rain')) {
                if (showToast) showToast('🌧️ Cuaca hujan — Janitor & Maintenance siaga!','warning');
            }
        } catch(e) { /* silent */ }
    }
    ...
    async function sendAIChat(msg) {
        ...
        chatEl.scrollTop = chatEl.scrollHeight;
        const systemPrompt = `Kamu adalah Dream AI Assistant untuk sistem manajemen Dream OS v2.0. `
        ...
        c.innerHTML = `
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
            ...
            <div style="display:flex;flex-direction:column;gap:.6rem">
              ...
              ${[['Booking','#10b981','cc2-sum-bk'],['K3','#f59e0b','cc2-sum-k3'],['Dana','#a855f7','cc2-sum-dn'],['SPJ','#ec4899','cc2-sum-spj']].map(item =>
    ...
    async function loadActivityFeed() {
        ...
        f.innerHTML = data.map(a =>
            `<div class="cc2-act-item"><div class="cc2-act-icon"><i class="fas fa-check-circle"></i></div>`
    ...
    async function loadPendingQueue() {
        ...
        html += section('#ec4899','file-invoice','SPJ',spj, s =>
            ...
            +`<div style="display:flex;gap:.3rem">`
            +`<button class="cc2-btn cc2-btn-em cc2-btn-sm" data-act="approve" data-tbl="spj" data-id="${s.id}" data-status="verified">Verifikasi</button>`
        ...
        v.innerHTML = `
          <div class="cc2-panel cc2-panel-purple">
            ...
            <div class="cc2-form-grid">
              <div class="cc2-form-group full"><label class="cc2-label">Judul Pengajuan *</label><input id="cc2-dana-judul" class="cc2-input" placeholder="Contoh: Pengadaan alat kebersihan" value="${esc(prefill.judul||'')}"></div>
              <div class="cc2-form-group"><label class="cc2-label">Jenis *</label><select id="cc2-dana-jenis" class="cc2-select">${DANA_TYPES.map(t=>`<option value="${t}">${t}</option>`).join('')}</select></div>
              ...
              <div class="cc2-form-group full"><label class="cc2-label">Uraian *</label><textarea id="cc2-dana-uraian" class="cc2-textarea" rows="4" placeholder="Jelaskan kebutuhan secara detail...">${esc(prefill.uraian||'')}</textarea></div>
              <div class="cc2-form-group"><label class="cc2-label">Lampiran</label><input id="cc2-dana-lampiran" class="cc2-input" placeholder="Link/nomor referensi" value="${esc(prefill.lampiran||'')}"></div>
    ...
    async function submitDana() {
        const judul    = (getEl('cc2-dana-judul')?.value||'').trim();
        const jenis    = getEl('cc2-dana-jenis')?.value||'';
        const tgl      = getEl('cc2-dana-tgl')?.value||'';
        const nominal  = parseFloat(getEl('cc2-dana-nominal')?.value||0);
        const pengaju  = (getEl('cc2-dana-pengaju')?.value||'').trim();
        const dept     = (getEl('cc2-dana-dept')?.value||'').trim();
        const uraian   = (getEl('cc2-dana-uraian')?.value||'').trim();
        const lampiran = (getEl('cc2-dana-lampiran')?.value||'').trim();
        const prioritas= getEl('cc2-dana-prioritas')?.value||'normal';
        if (!judul||!pengaju||!uraian||!nominal) { if (showToast) showToast('Lengkapi semua field wajib (*)','error'); return; }
        if (nominal<=0) { if (showToast) showToast('Nominal harus lebih dari 0','error'); return; }
        const btn = getEl('cc2-dana-submit');
        if (btn) { btn.disabled=true; btn.innerHTML='<i class="fas fa-circle-notch" style="animation:cc2spin 1s linear infinite"></i> Mengirim...'; }
        if (!_sb) { if (showToast) showToast('Database tidak tersedia','error'); if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>Kirim Pengajuan';} return; }
        const res = await _sb.from('pengajuan_dana').insert([{
            judul, jenis, tanggal_butuh: tgl, nominal, pengaju, departemen: dept,
            uraian, lampiran, prioritas, status: 'pending',
            created_at: new Date().toISOString()
        }]);
        if (res.error) { if (showToast) showToast('❌ Gagal: '+res.error.message,'error'); if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>Kirim Pengajuan';} return; }
        await writeAuditLog('Pengajuan Dana', judul+' · '+fmtRp(nominal), pengaju);
        if (showToast) showToast('✅ Pengajuan dana berhasil dikirim!','success');
        loadStats();
        renderDanaList();
    }
    ...
    async function renderDanaList() {
        ...
        const isAdmin = _user && (_user.perms||[]).includes('all');
        let query = _sb.from('pengajuan_dana').select('*').order('created_at',{ascending:false}).limit(30);
        ...
        v.innerHTML = statusBtns
            +'<div class="cc2-tbl-wrap"><table class="cc2-tbl"><thead><tr><th>Judul</th><th>Nominal</th><th>Pengaju</th><th>Prioritas</th><th>Status</th><th>Tanggal</th><th>Aksi</th></tr></thead>'
            +`<tbody id="cc2-dana-tbody">${tblRows}</tbody></table></div>`;
        [['cc2-df-all',null],['cc2-df-pend','pending'],['cc2-df-appr','approved'],['cc2-df-rej','rejected']].forEach(([id,st]) => {
            getEl(id)?.addEventListener('click', () => {
                document.querySelectorAll('#cc2-dana-tbody tr').forEach(tr => {
    ...
    function renderSPJForm(prefill) {
        ...
        v.innerHTML = `
          <div class="cc2-panel" style="border-color:rgba(236,72,153,.3)">
            ...
            <div class="cc2-form-grid">
              <div class="cc2-form-group full"><label class="cc2-label">Judul SPJ *</label><input id="cc2-spj-judul" class="cc2-input" placeholder="Contoh: SPJ Upacara Kemerdekaan" value="${esc(prefill.judul||'')}"></div>
              <div class="cc2-form-group"><label class="cc2-label">Kategori *</label><select id="cc2-spj-kategori" class="cc2-select">${SPJ_CATS.map(c=>`<option value="${c}">${c}</option>`).join('')}</select></div>
              <div class="cc2-form-group"><label class="cc2-label">No. Referensi Dana</label><input id="cc2-spj-refno" class="cc2-input" placeholder="Nomor pengajuan terkait" value="${esc(prefill.nomor_ref||'')}"></div>
              <div class="cc2-form-group"><label class="cc2-label">Tanggal Kegiatan *</label><input id="cc2-spj-tgl" class="cc2-input" type="date" value="${prefill.tanggal_kegiatan||new Date().toISOString().split('T')[0]}"></div>
              ...
              <div class="cc2-form-group full"><label class="cc2-label">Uraian *</label><textarea id="cc2-spj-uraian" class="cc2-textarea" rows="4" placeholder="Jelaskan kegiatan dan penggunaan dana...">${esc(prefill.uraian||'')}</textarea></div>
              <div class="cc2-form-group full">
                <label class="cc2-label">Rincian Item</label>
              ...
              <div class="cc2-form-group"><label class="cc2-label">Bukti/Kwitansi</label><input id="cc2-spj-bukti" class="cc2-input" placeholder="Link foto/scan kwitansi" value="${esc(prefill.bukti||'')}"></div>
            </div>
            <hr class="cc2-divider">
            <div style="display:flex;gap:.5rem;justify-content:flex-end;flex-wrap:wrap">
              <button class="cc2-btn cc2-btn-bl" id="cc2-spj-cancel"><i class="fas fa-times"></i>Batal</button>
              <button class="cc2-btn cc2-btn-or" id="cc2-spj-preview-btn"><i class="fas fa-eye"></i>Preview</button>
        ...
        const itemsEl = getEl('cc2-spj-items');
        let itemCount = 0;
        function addItem(){
            itemCount++;
            const row = document.createElement('div');
            row.style.cssText='display:flex;gap:.4rem;margin-bottom:.35rem;align-items:center';
            row.innerHTML=`<input class="cc2-input" style="flex:2" placeholder="Nama item" data-item-name="${itemCount}"><div class="cc2-input-rp" style="flex:1"><span>Rp</span><input type="number" placeholder="0" data-item-val="${itemCount}"></div><button class="cc2-btn cc2-btn-re cc2-btn-sm" type="button" onclick="this.parentElement.remove()"><i class="fas fa-trash"></i></button>`;
        ...
        ['cc2-spj-approved','cc2-spj-total'].forEach(id => {
            getEl(id)?.addEventListener('input', () => {
                const approved = parseFloat(getEl('cc2-spj-approved')?.value||0);
                const total    = parseFloat(getEl('cc2-spj-total')?.value||0);
                const sisaEl   = getEl('cc2-spj-sisa');
    ...
    function previewSPJ() {
        const judul   = getEl('cc2-spj-judul')?.value||'—';
        const pengaju = getEl('cc2-spj-pengaju')?.value||'—';
        const tgl     = getEl('cc2-spj-tgl')?.value||'—';
        const total   = fmtRp(getEl('cc2-spj-total')?.value||0);
        const approved= fmtRp(getEl('cc2-spj-approved')?.value||0);
        const sisa    = fmtRp(getEl('cc2-spj-sisa')?.value||0);
        const uraian  = getEl('cc2-spj-uraian')?.value||'—';
        const kat     = getEl('cc2-spj-kategori')?.value||'—';
        const items   = [];
        ...
        modal.innerHTML=`<div style="background:white;color:#1e293b;padding:2rem;border-radius:14px;max-width:640px;width:100%;max-height:90vh;overflow-y:auto;font-family:Georgia,serif">
          ...
          <div style="display:flex;gap:.5rem;justify-content:center;margin-top:1.5rem;flex-wrap:wrap">
            ...
            <button id="cc2-modal-close" style="background:#e2e8f0;color:#1e293b;padding:.5rem 1.2rem;border:none;border-radius:8px;font-weight:700;cursor:pointer">✕ Tutup</button>
          </div>
        </div>`;
        document.body.appendChild(modal);
        modal.querySelector('#cc2-modal-close').addEventListener('click', () => modal.remove());
        modal.addEventListener('click', e => { if(e.target===modal) modal.remove(); });
    }

    async function submitSPJ() {
        const judul   = (getEl('cc2-spj-judul')?.value||'').trim();
        const kat     = getEl('cc2-spj-kategori')?.value||'';
        const tgl     = getEl('cc2-spj-tgl')?.value||'';
        const total   = parseFloat(getEl('cc2-spj-total')?.value||0);
        const approved= parseFloat(getEl('cc2-spj-approved')?.value||0);
        const pengaju = (getEl('cc2-spj-pengaju')?.value||'').trim();
        const uraian  = (getEl('cc2-spj-uraian')?.value||'').trim();
        const bukti   = (getEl('cc2-spj-bukti')?.value||'').trim();
        const sisa    = parseFloat(getEl('cc2-spj-sisa')?.value||0);
        const refno   = (getEl('cc2-spj-refno')?.value||'').trim();
        if (!judul||!pengaju||!uraian||!total) { if (showToast) showToast('Lengkapi semua field wajib (*)','error'); return; }
        const items = [];
        document.querySelectorAll('[data-item-name]').forEach(inp => {
            const valEl = document.querySelector(`[data-item-val="${inp.dataset.itemName}"]`);
            const name  = inp.value.trim();
        ...
        const btn = getEl('cc2-spj-submit');
        if (btn) { btn.disabled=true; btn.innerHTML='<i class="fas fa-circle-notch" style="animation:cc2spin 1s linear infinite"></i>...'; }
        if (!_sb) { if (showToast) showToast('Database tidak tersedia','error'); if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>Submit SPJ';} return; }
        const nomor = 'SPJ-'+new Date().getFullYear()+'-'+String(Date.now()).slice(-5);
        const res = await _sb.from('spj').insert([{
        ...
        if (res.error) { if (showToast) showToast('❌ Gagal: '+res.error.message,'error'); if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>Submit SPJ';} return; }
        await writeAuditLog('Submit SPJ', nomor+' · '+judul+' · '+fmtRp(total), pengaju);
        if (showToast) showToast('✅ SPJ '+nomor+' berhasil dikirim!','success');
        loadStats();
        renderSPJList();
    }
    ...
    async function renderSPJList() {
        ...
        const isAdmin = _user && (_user.perms||[]).includes('all');
        let query = _sb.from('spj').select('*').order('created_at',{ascending:false}).limit(30);
        if (!isAdmin && _user) query = query.eq('pengaju', _user.name);
        const res = await query;
        const data = res.data||[];
        if (!data.length) { v.innerHTML='<p style="text-align:center;padding:2rem;opacity:.45">Belum ada SPJ yang diajukan</p>'; return; }
        v.innerHTML='<div class="cc2-tbl-wrap"><table class="cc2-tbl"><thead><tr><th>No. SPJ</th><th>Judul</th><th>Total Biaya</th><th>Pengaju</th><th>Status</th><th>Tanggal</th><th>Aksi</th></tr></thead><tbody>'
            + data.map(s => {
                const bdg = s.status==='pending'?'<span class="cc2-badge cc2-b-pend">Pending</span>':s.status==='verified'||s.status==='approved'?'<span class="cc2-badge cc2-b-appr">Diverifikasi</span>':'<span class="cc2-badge cc2-b-rej">Ditolak</span>';
    ...
    function renderActivity() {
        const c = getEl('cc2-content');
        c.innerHTML = `<h3 style="font-size:.95rem;font-weight:800;margin-bottom:1rem;color:#3b82f6"><i class="fas fa-history" style="margin-right:.5rem"></i>Log Aktivitas Sistem</h3><div id="cc2-full-log" class="cc2-feed" style="max-height:480px">${mkLoader()}</div>`;
        loadFullLog();
    }
    ...
    async function loadFullLog() {
        ...
        c.innerHTML = data.map(a =>
            `<div class="cc2-act-item"><div class="cc2-act-icon"><i class="fas fa-history"></i></div>`
            +`<div><div class="cc2-act-title">${esc(a.action)}</div>`
            +`<div class="cc2-act-meta">${esc(a.detail||'')}${a.user?' · '+esc(a.user):''}${a.created_at?' · '+fmtDT(a.created_at):''}</div></div></div>`
        ).join('');
    }
    ...
    function renderAnalytics() {
        ...
        c.innerHTML = `
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1rem">
            ...
            <div class="cc2-panel" style="padding:.875rem"><p style="font-weight:700;font-size:.82rem;margin-bottom:.625rem">📈 Trend 7 Hari (Booking)</p><div class="cc2-chart-box"><canvas id="cc2-ch-trend"></canvas></div></div>
          </div>
          <div class="cc2-panel" style="margin-top:1rem;padding:.875rem">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:.75rem">
              ${[['Total Dana Pending','#a855f7','cc2-an-dpend'],['Dana Disetujui','#10b981','cc2-an-dappr'],['Total SPJ Bulan Ini','#ec4899','cc2-an-spjmon'],['Sisa Dana','#3b82f6','cc2-an-sisa']].map(item=>`<div class="cc2-stat"><div class="cc2-sv" style="color:${item[1]};font-size:1.2rem" id="${item[2]}">—</div><div class="cc2-sl">${item[0]}</div></div>`).join('')}
            </div>
          </div>`;
    ...
    function buildCharts() {
        const ctxDist = getEl('cc2-ch-dist');
        if (ctxDist) {
        ...
        const ctxTrend = getEl('cc2-ch-trend');
        if (ctxTrend) {
    ...
    function renderSystem() {
        const c = getEl('cc2-content');
        const metrics = [
            ['Database','#10b981',98,'cc2-h-db'],['API Response','#10b981',100,'cc2-h-api'],
            ['Storage','#f59e0b',73,'cc2-h-stor'],['Security Score','#10b981',100,'cc2-h-sec'],['Uptime','#3b82f6',99,'cc2-h-up']
        ];
        c.innerHTML = `
          ...
          <div style="display:flex;flex-direction:column;gap:.55rem;margin-bottom:1rem">
            ${metrics.map(m=>`<div class="cc2-panel" style="padding:.75rem"><div style="display:flex;justify-content:space-between;margin-bottom:.3rem;font-size:.78rem"><span style="color:#94a3b8"><i class="fas fa-circle" style="font-size:.5rem;color:${m[1]};margin-right:.4rem"></i>${m[0]}</span><span style="font-family:'JetBrains Mono',monospace;color:${m[1]};font-weight:700" id="${m[3]}">${m[2]}%</span></div><div class="cc2-hbar"><div class="cc2-hfill" style="width:${m[2]}%;background:${m[1]}"></div></div></div>`).join('')}
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:1rem">
            ...
            <div class="cc2-panel" style="padding:.875rem"><p style="font-size:.7rem;color:#64748b;text-transform:uppercase;font-weight:700;margin-bottom:.3rem">Supabase</p><p id="cc2-sb-status" style="font-family:monospace;color:#10b981;font-weight:700">CONNECTED</p></div>
    ...
    function runDiagnostic() {
        if (showToast) showToast('🔍 Running system diagnostic...','info');
        setTimeout(() => {
            ...
            if (showToast) showToast('✅ Diagnostic selesai — Semua sistem normal','success');
        }, 1800);
    }
    ...
    async function doBackup() {
        ...
        try {
            ...
            if (showToast) showToast('✅ Backup berhasil diunduh!','success');
        } catch(e) { if (showToast) showToast('❌ Backup gagal: '+e.message,'error'); }
    }

    async function doExportCSV() {
        if (!_sb) { if (showToast) showToast('DB tidak tersedia','error'); return; }
        if (showToast) showToast('📊 Mengekspor data...','info');
        try {
            const res  = await _sb.from('pengajuan_dana').select('*').order('created_at',{ascending:false});
            const data = res.data||[];
            if (!data.length) { if (showToast) showToast('Tidak ada data untuk diekspor','warning'); return; }
            ...
            a.click();
            URL.revokeObjectURL(a.href);
            if (showToast) showToast('✅ CSV berhasil diekspor!','success');
    ...
    async function handleApprove(tbl, id, status) {
        ...
        if (showToast) showToast('✅ Status diperbarui: '+status,'success');
        loadStats();
    ...
    function bindEvents() {
        document.querySelectorAll('.cc2-tab').forEach(btn => {
            btn.addEventListener('click', () => switchTab(btn.dataset.tab));
        });
        const aiToggle = getEl('cc2-ai-toggle');
        const aiBody   = getEl('cc2-ai-body');
        if (aiToggle && aiBody) {
        ...
        const aiSend = getEl('cc2-ai-send');
        const aiInp  = getEl('cc2-ai-inp');
        if (aiSend && aiInp) {
        ...
        const content = getEl('cc2-content');
    ...
    function subscribeRealtime() {
        ...
        try {
            _channel = _sb.channel('cc2-live')
                .on('postgres_changes',{event:'INSERT',schema:'public',table:'bookings'},      () => { loadStats(); if (showToast) showToast('📅 Booking baru masuk!','info'); })
                .on('postgres_changes',{event:'INSERT',schema:'public',table:'k3_reports'},    () => { loadStats(); if (showToast) showToast('⚠️ Laporan K3 baru!','warning'); })
                .on('postgres_changes',{event:'INSERT',schema:'public',table:'pengajuan_dana'},() => { loadStats(); if (showToast) showToast('💰 Pengajuan dana baru!','info'); })
                .on('postgres_changes',{event:'INSERT',schema:'public',table:'spj'},           () => { loadStats(); if (showToast) showToast('📋 SPJ baru masuk!','info'); })
