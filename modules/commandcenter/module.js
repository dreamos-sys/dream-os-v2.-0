📁 File modules/commandcenter/module.js dengan Integrasi Cuaca (OpenWeatherMap)

BRO, ini dia file module.js yang sudah ditambahkan fitur cuaca menggunakan API key dari OpenWeatherMap. Sekarang AI Insight Panel akan menampilkan cuaca terkini, dan setiap divisi (Umum, Janitor, Maintenance, Sekuriti) mendapat notifikasi sesuai kondisi cuaca. Slide 4 (Cuaca & Mitigasi) juga otomatis ter-update.
...
const CITY = 'Depok'; // Bisa diganti dengan kota yang diinginkan
...
async function checkWeatherAndNotify() {
    try {
        ...
        // Update AI Insight & Slide 4
        updateWeatherInsight(alert);
        updateWeatherSlide(alert);

        // Kirim notifikasi jika ada perubahan signifikan
        if (lastWeatherAlert) {
...
function updateWeatherSlide(alert) {
    if (window.refreshSlider) {
        ...
        // Fallback: update langsung elemen slide
...
async function logWeatherAlert(alert) {
    ...
    try {
        ...
        console.error('Gagal log weather:', err);
    }
}
...
async function loadAllStats() {
    ...
    try {
        ...
        // generateAIInsights sudah digantikan oleh weather, jadi kita tidak panggil lagi
...
· API Key: f7890d7569950ffa34a5827880e8442f (sudah dimasukkan).
· Kota: Depok (bisa diganti sesuai lokasi).
· Interval update: 30 menit (bisa diubah).
· AI Insight Panel sekarang menampilkan cuaca terkini.
· Notifikasi dikirim ke divisi Umum, Janitor, Maintenance, Sekuriti jika ada perubahan signifikan.
· Slide 4 diperbarui secara otomatis (via window.refreshSlider jika ada).

Simpan file ini sebagai modules/commandcenter/module.js dan pastikan file template.js (HTML) sudah ada di folder yang sama. Selesai! 🚀💚
